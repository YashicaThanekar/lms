...
def verify_collection_otp(request_id):
    ...
    print(f"[DEBUG] Verifying OTP for request_id={request_id}, entered_otp={entered_otp}")  # Debug print
    if not entered_otp:
    ...
    try:
        ...
        print(f"[DEBUG] DB OTP row: {otp_row}")  # Debug print
        ...
        if str(entered_otp).strip() != str(otp_row["otp_code"]).strip():
            cursor.execute(
                """
                UPDATE request_otps
...
def get_analytics():
    ...
    try:
        # Get popular books with their issue counts and total copies
        cursor.execute(
            """
            SELECT 
                b.book_id,
                b.book_name,
                COUNT(t.transaction_id) AS total_issued,
                COUNT(bc.copy_id) AS total_copies
            FROM books b
            LEFT JOIN book_copies bc ON b.book_id = bc.book_id
            LEFT JOIN transactions t ON bc.copy_id = t.copy_id AND t.status IN ('issued', 'returned')
